...
public class JdbcTemplate {
    public <T> T selectOne(String sql, RowMapper rowMapper, Object... objects) throws SQLException {
        ResultSet rs = null;
        try (Connection con = ConnectionManager.getConnection();
             PreparedStatement pstmt = con.prepareStatement(sql)) {
            prepareValues(pstmt, objects);
            rs = pstmt.executeQuery();
            return (T) rowMapper.mapResult(rs);
        } finally {
            if (rs != null) {
                rs.close();
            }
    ...
    public <T> T selectAll(String sql, RowMapper rowMapper) throws SQLException {
        ResultSet rs = null;
        try (Connection con = ConnectionManager.getConnection();
             PreparedStatement pstmt = con.prepareStatement(sql)) {
            rs = pstmt.executeQuery();
